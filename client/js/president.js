...
function main() {
    d3.csv("{{site.baseurl}}/data/president-history.csv?" + Math.random(), d => ({
...
function makeMap(showProbabilities = true) {
    ...
    queue()
        .defer(d3.json, "{{site.baseurl}}/assets/usa.json")
        .defer(d3.csv, "{{site.baseurl}}/data/president-states.csv?" + Math.random(), d => ({ 
...
function makeHistogram(mean) {
    ...
    d3.csv("{{site.baseurl}}/data/president-electors.csv?" + Math.random(), d => ({
        electors: +d.electors,
